...
const target = `{savedMaterials.map(m => (
                      <button
                        key={m.id}
...
const replace = `{savedMaterials.map(m => (
...
if (content.includes(target)) {
  ...
  console.log("Still could not find it. Writing the target to a debug file.");
  fs.writeFileSync('debug_target.txt', target);
}
